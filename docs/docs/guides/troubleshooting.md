...
| Freeze at "X minutes remaining" during install, CPU/RAM pegged, mouse/keyboard unresponsive | On Xeon or pre-Skylake Intel hosts, the `vmxnet3` NIC kext often fails to load during install, stalling the copy phase | Switch to `e1000-82545em`: `qm set <vmid> --net0 e1000-82545em,bridge=<bridge>,firewall=0`. The TUI and bash installer auto-detect this and pick `e1000-82545em` by default on those CPUs. |
